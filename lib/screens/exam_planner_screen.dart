...
List<StudyItem> generateExamPlan({
  ...
  final weights = uniqueSubjects.map((subject) => (priorities[subject] ?? 2).clamp(1, 3).toInt() + safeUrgency - 1).toList();
  ...
  if (blockCount > 0 && totalWeight > 0) {
    ...
    for (var i = 0; i < remainingBlocks; i++) counts[order[i % order.length]]++;
  }
  final items = <StudyItem>[];
  for (var i = 0; i < uniqueSubjects.length; i++) {
    for (var b = 0; b < counts[i]; b++) {
      items.add(StudyItem(title: uniqueSubjects[i], minutes: 25, topic: nextDay ? 'High-impact revision' : 'Exam preparation'));
    }
  }
  ...
  if (remainder > 0) {
    ...
    for (var i = 1; i < uniqueSubjects.length; i++) {
      if (weights[i] > weights[remainderTarget]) remainderTarget = i;
    }
    items.add(StudyItem(title: uniqueSubjects[remainderTarget], minutes: remainder, topic: nextDay ? 'Final review' : 'Flexible review'));
  }
  return items;
}
...
class _ExamPlannerScreenState extends State<ExamPlannerScreen> {
  ...
  List<StudyItem> _generatePlan() => generateExamPlan(nextDay: widget.nextDay, studyHours: studyHours, urgency: urgency, subjects: subjects, priorities: priorities);
        ...
        child: Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text(widget.nextDay ? 'Tomorrow is ready' : 'Your exam plan is ready', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)),
                ...
                Container(
                  ...
                  child: Row(children: [Icon(widget.nextDay ? Icons.bolt_rounded : Icons.auto_awesome_rounded), const SizedBox(width: 10), Expanded(child: Text(widget.nextDay ? 'High-priority subjects get more focus blocks.' : 'Priority and exam urgency shape the distribution.'))]),
                ),
      ...
      body: ListView(
        ...
        children: [
          ...
          Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8),
            SegmentedButton<int>(segments: const [ButtonSegment(value: 1, label: Text('Low')), ButtonSegment(value: 2, label: Text('Medium')), ButtonSegment(value: 3, label: Text('High'))], selected: {urgency}, onSelectionChanged: (value) => setState(() => urgency = value.first)),
          ]))),
          const SizedBox(height: 18),
          Text('Subjects', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900)),
          const SizedBox(height: 8),
          Row(children: [Expanded(child: TextField(controller: controller, onSubmitted: (_) => addSubject(), decoration: const InputDecoration(hintText: 'e.g. Physics', prefixIcon: Icon(Icons.menu_book_rounded)))), const SizedBox(width: 8), IconButton.filled(onPressed: addSubject, icon: const Icon(Icons.add_rounded))]),
          const SizedBox(height: 10),
          ...subjects.map((subject) => Card(margin: const EdgeInsets.only(bottom: 8), child: ListTile(
            title: Text(subject, style: const TextStyle(fontWeight: FontWeight.w800)),
            subtitle: DropdownButton<int>(value: priorities[subject] ?? 2, isExpanded: true, underline: const SizedBox.shrink(), items: const [DropdownMenuItem(value: 1, child: Text('Low priority')), DropdownMenuItem(value: 2, child: Text('Medium priority')), DropdownMenuItem(value: 3, child: Text('High priority'))], onChanged: (value) => setState(() => priorities[subject] = value ?? 2)),
