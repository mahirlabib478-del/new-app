...
class AttractiveHome extends StatelessWidget {
  ...
  void _tap(VoidCallback action) { SoundEffects(store).tap(); action(); }
    ...
    return SafeArea(child: ListView(padding: const EdgeInsets.fromLTRB(20, 18, 20, 28), children: [
      ...
      if (snapshot.plan == null)
        ...
        ...snapshot.plan!.items.asMap().entries.map((entry) {
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 10), child: _JourneyItem(item: item, progress: progress, active: active, complete: complete, minutesLabel: strings.minutes, onTap: active ? () => _tap(() => onOpenFocus()) : null));
        }),
      ...
      Card(child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [Icon(Icons.insights_rounded, color: scheme.primary), const SizedBox(width: 12), Expanded(child: Text(snapshot.recommendationReason, style: const TextStyle(fontWeight: FontWeight.w700)))]))),
    ]));
  }
...
class _MissionCard extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(padding: const EdgeInsets.all(22), decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [scheme.primaryContainer, scheme.secondaryContainer])), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      FilledButton.icon(key: const ValueKey<String>('home-mission-action'), onPressed: onStart, icon: Icon(snapshot.isComplete ? Icons.check_rounded : Icons.play_arrow_rounded), label: Text(snapshot.isComplete ? (strings.isBangla ? 'নতুন প্ল্যান' : 'New plan') : (strings.isBangla ? 'শুরু করুন' : 'Start'))),
    ]));
  }
}
...
class _GoalCard extends StatelessWidget {
  ...
  final TodaySnapshot snapshot; final AppStrings strings;
  @override Widget build(BuildContext context) {
    final progress = snapshot.goalProgress.clamp(0.0, 1.0).toDouble();
    return Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Expanded(child: Text(strings.isBangla ? 'আজকের লক্ষ্য' : 'Today’s goal', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900))), Text('${snapshot.todayCompletedMinutes} / ${snapshot.dailyGoalMinutes} ${strings.minutes}', style: Theme.of(context).textTheme.labelLarge?.copyWith(fontWeight: FontWeight.w800))]), const SizedBox(height: 10), LinearProgressIndicator(value: progress, minHeight: 8), const SizedBox(height: 8), Text(snapshot.dailyGoalReached ? (strings.isBangla ? 'লক্ষ্য পূর্ণ। গতি ধরে রাখুন।' : 'Goal reached. Keep the momentum.') : '${snapshot.goalRemainingMinutes} ${strings.minutes} ${strings.isBangla ? 'বাকি আজ' : 'left today'}')]));
  }
}
...
class _JourneyItem extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) { final scheme = Theme.of(context).colorScheme; return Card(child: ListTile(onTap: onTap, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), leading: CircleAvatar(backgroundColor: complete ? scheme.primary : scheme.surfaceContainerHighest, foregroundColor: complete ? scheme.onPrimary : scheme.onSurfaceVariant, child: Icon(complete ? Icons.check_rounded : active ? Icons.play_arrow_rounded : Icons.menu_book_rounded)), title: Text(item.title, style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [if (item.topic.isNotEmpty) Text(item.topic), const SizedBox(height: 6), LinearProgressIndicator(value: progress)]), trailing: active ? const Icon(Icons.chevron_right_rounded) : Text('$minutesLabel ${item.minutes}'))); }
}
