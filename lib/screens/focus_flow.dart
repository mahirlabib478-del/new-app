...
class _BreakScreenState extends State<BreakScreen> with WidgetsBindingObserver {
  ...
  @override void initState() { super.initState(); WidgetsBinding.instance.addObserver(this); var expiredOnResume = false; final saved = widget.store.breakTimerState; if (saved != null && saved.index == widget.index && saved.blockIndex == widget.blockIndex) { breakMinutes = saved.breakMinutes == 10 ? 10 : 5; if (saved.running && saved.deadlineMillis != null) { final remainingSeconds = ((saved.deadlineMillis! - DateTime.now().millisecondsSinceEpoch) / 1000).ceil(); seconds = remainingSeconds.clamp(0, breakMinutes * 60).toInt(); running = true; expiredOnResume = remainingSeconds <= 0; } else { seconds = saved.remainingSeconds.clamp(0, breakMinutes * 60).toInt(); running = false; } } unawaited(StudySessionStore(widget.store).archiveCurrentPlan()); if (expiredOnResume) WidgetsBinding.instance.addPostFrameCallback((_) => _next()); else { _persistBreakState(); if (running) _startTimer(); } }
  void _startTimer() { timer?.cancel(); timer = Timer.periodic(const Duration(seconds: 1), (_) { if (!mounted || !running || advancing) return; if (seconds > 0) setState(() => seconds--); if (seconds == 0) { timer?.cancel(); unawaited(widget.store.clearBreakTimerState()); unawaited(_next()); } }); }
  void _persistBreakState() { if (advancing) return; final deadline = running ? DateTime.now().add(Duration(seconds: seconds)).millisecondsSinceEpoch : null; unawaited(widget.store.saveBreakTimerState(BreakTimerState(index: widget.index, blockIndex: widget.blockIndex, breakMinutes: breakMinutes, remainingSeconds: seconds, running: running, deadlineMillis: deadline))); }
  @override void didChangeAppLifecycleState(AppLifecycleState state) { if (state == AppLifecycleState.paused || state == AppLifecycleState.inactive) { _persistBreakState(); unawaited(StudySessionStore(widget.store).archiveCurrentPlan()); } else if (state == AppLifecycleState.resumed && mounted && running && !advancing) { final saved = widget.store.breakTimerState; if (saved != null && saved.index == widget.index && saved.blockIndex == widget.blockIndex && saved.deadlineMillis != null) { final nextSeconds = ((saved.deadlineMillis! - DateTime.now().millisecondsSinceEpoch) / 1000).ceil(); if (nextSeconds <= 0) { seconds = 0; timer?.cancel(); unawaited(widget.store.clearBreakTimerState()); unawaited(_next()); return; } setState(() => seconds = nextSeconds.clamp(0, breakMinutes * 60).toInt()); } _startTimer(); } }
  void _setBreakMinutes(int minutes) { if (advancing || seconds < breakMinutes * 60 - 5) return; setState(() { breakMinutes = minutes; seconds = minutes * 60; }); _persistBreakState(); }
  void _toggleRunning() { if (advancing || seconds <= 0) return; setState(() => running = !running); _persistBreakState(); unawaited(StudySessionStore(widget.store).archiveCurrentPlan()); if (running) _startTimer(); else timer?.cancel(); }
