...
class ExamPlannerScreen extends StatefulWidget {
  const ExamPlannerScreen({super.key, required this.nextDay});
  final bool nextDay;
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(20), children: [
        ...
        Text(widget.nextDay ? 'Prioritize the most important topics and use every focused block wisely.' : 'Set your available time, then rank subjects by importance.'),
        ...
        const SizedBox(height: 24),
        FilledButton.icon(onPressed: subjects.isEmpty ? null : () => showDialog(context: context, builder: (_) => AlertDialog(title: const Text('Plan ready'), content: Text('Your $studyHours-hour plan will prioritize high-importance subjects and use 25-minute focus blocks.'), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Got it'))])), icon: const Icon(Icons.auto_awesome_rounded), label: const Padding(padding: EdgeInsets.all(14), child: Text('Generate exam plan'))),
      ]),
    );
  }
}
