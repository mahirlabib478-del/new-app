...
class _RegularStudyPlannerState extends State<RegularStudyPlanner> {
  ...
  void addTopic(String subject) {
    ...
    if (name.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter a chapter or topic name first.'))); return; }
    if (topics[subject]!.any((t) => t.name.toLowerCase() == name.toLowerCase())) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('That topic is already added.'))); return; }
  ...
  Future<void> save() async {
    ...
    if (items.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Allocate at least 1 minute to a topic or subject.'))); return; }
    if (hasUnassigned && mounted) {
      final proceed = await showDialog<bool>(context: context, builder: (context) => AlertDialog(
        ...
        actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Keep editing')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Save plan'))],
      ));
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.fromLTRB(20, 8, 20, 28), children: [
        ...
        Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          LinearProgressIndicator(value: widget.total == 0 ? 0 : allocated / widget.total, minHeight: 9),
          const SizedBox(height: 8),
          Text(remaining == 0 ? 'Fully allocated — ready to focus.' : '$remaining minutes available to assign', style: TextStyle(fontWeight: FontWeight.w700, color: remaining == 0 ? scheme.primary : null)),
        ]))),
  ...
  Widget _subjectCard(String subject, int? totalDivisions) {
    ...
    return Card(margin: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (list.isNotEmpty) ...[
        ...
        ...list.map((topic) => Padding(padding: const EdgeInsets.only(bottom: 8), child: Container(
