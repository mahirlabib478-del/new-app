...
class _ExamPlannerScreenState extends State<ExamPlannerScreen> {
  ...
  void addSubject() {
    ...
    if (value.isNotEmpty && !subjects.contains(value)) {
      setState(() { subjects.add(value); priorities[value] = 2; controller.clear(); });
    }
  }
  ...
  List<StudyItem> _generatePlan() {
    ...
    for (var i = 0; i < subjects.length; i++) counts[i] = (blocks * weights[i] / weightTotal).floor();
    var assigned = counts.fold(0, (a, b) => a + b);
    final ranked = List<int>.generate(subjects.length, (i) => i)..sort((a, b) => weights[b].compareTo(weights[a]));
    var cursor = 0;
    while (assigned < blocks) { counts[ranked[cursor % ranked.length]]++; assigned++; cursor++; }
    final items = <StudyItem>[];
    for (var i = 0; i < subjects.length; i++) {
      for (var b = 0; b < counts[i]; b++) {
        items.add(StudyItem(title: subjects[i], minutes: 25, topic: widget.nextDay ? 'High-impact revision' : 'Exam preparation'));
      }
    }
    if (remainder > 0) items.add(StudyItem(title: subjects[ranked.first], minutes: remainder, topic: widget.nextDay ? 'Final review' : 'Flexible review'));
    return items;
  }
    ...
    showModalBottomSheet<void>(
      ...
      builder: (sheetContext) => SafeArea(child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 6), Text('${items.length} focus blocks • ${studyHours}h total • 25-minute focus cycles'),
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: Text(title)), body: ListView(padding: const EdgeInsets.all(20), children: [
      ...
      const SizedBox(height: 8), Text(widget.nextDay ? 'Prioritize the most important subjects and use every focused block wisely.' : 'Set your available time, then rank subjects by importance.'),
      ...
      const SizedBox(height: 16),
      Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('EXAM URGENCY', style: TextStyle(fontWeight: FontWeight.w800, letterSpacing: 1)), const SizedBox(height: 10),
      ...
      const SizedBox(height: 16), const Text('SUBJECTS', style: TextStyle(fontWeight: FontWeight.w800, letterSpacing: 1)), const SizedBox(height: 8),
      ...
      const SizedBox(height: 24), FilledButton.icon(onPressed: subjects.isEmpty ? null : _showGeneratedPlan, icon: const Icon(Icons.auto_awesome_rounded), label: const Padding(padding: EdgeInsets.all(14), child: Text('Generate exam plan'))),
    ]));
  }
}