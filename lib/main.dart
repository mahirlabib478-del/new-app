...
class _StudyOSState extends State<StudyOS> {
  ...
  Widget build(BuildContext context) => MaterialApp(
    ...
    home: Scaffold(
      body: [
        Home(store: widget.store, onTheme: () => setTheme(themeKey == 'midnight' ? 'sunrise' : 'midnight'), onOpenFocus: openFocus),
        StudyHub(store: widget.store, onStartPlan: (plan) => openFocus(plan: plan)),
...
class Home extends StatelessWidget {
  const Home({super.key, required this.store, required this.onTheme, required this.onOpenFocus});
  final LocalStore store;
  ...
  Widget build(BuildContext context) {
    final s = TodayEngine(store).build();
    final plan = s.plan;
    final canContinue = s.hasPlan && s.remainingMinutes > 0;
    return Scaffold(
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            if (s.nextItem != null) _NextUp(snapshot: s, onStart: () => onOpenFocus()),
            if (s.nextItem != null) const SizedBox(height: 20),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('Study modes', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
              Text('${s.streak} day streak', style: Theme.of(context).textTheme.labelLarge),
            ]),
            ...
            _Mode(icon: Icons.auto_awesome_rounded, title: 'Exam Preparation', subtitle: 'Build a focused exam plan around your priorities.', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ExamPlannerScreen(nextDay: false, store: store, onStartPlan: (p) async {
              if (!context.mounted) return;
              Navigator.pop(context);
              await onOpenFocus(plan: p);
            })))),
            _Mode(icon: Icons.bolt_rounded, title: 'Next Day Exam', subtitle: 'Prioritize the highest-impact revision for tomorrow.', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ExamPlannerScreen(nextDay: true, store: store, onStartPlan: (p) async {
              if (!context.mounted) return;
              Navigator.pop(context);
              await onOpenFocus(plan: p);
            })))),
              ...
              child: plan == null
                  ...
                  : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        ...
                        Expanded(child: Text('${plan.items.length} focus blocks planned', style: const TextStyle(fontWeight: FontWeight.w900))),
                        Text('${s.completedMinutes}/${plan.allocatedMinutes}m', style: const TextStyle(fontWeight: FontWeight.w800)),
                      ]),
                      const SizedBox(height: 14),
                      LinearProgressIndicator(value: s.progress, minHeight: 8),
                      const SizedBox(height: 10),
                      Text(s.remainingMinutes == 0 ? 'Plan complete — great work.' : '${s.remainingMinutes} minutes remaining'),
                      const SizedBox(height: 12),
                      SizedBox(
                        width: double.infinity,
                        child: OutlinedButton.icon(
                          onPressed: canContinue ? () => onOpenFocus() : null,
                          icon: Icon(s.completedMinutes == 0 ? Icons.play_arrow_rounded : Icons.play_circle_outline_rounded),
                          label: Text(canContinue ? (s.completedMinutes == 0 ? 'Start today\'s plan' : 'Resume plan') : 'Plan completed'),
                        ),
                      ),
                    ]),
            )),
  ...
  Widget build(BuildContext context) {
    ...
    return TweenAnimationBuilder<double>(
      ...
      builder: (_, value, __) => Container(
        ...
        decoration: BoxDecoration(gradient: LinearGradient(colors: [c.primary, c.secondary]), borderRadius: BorderRadius.circular(28)),
...
class _Mode extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
    ...
    child: ListTile(onTap: onTap, contentPadding: const EdgeInsets.all(12), leading: CircleAvatar(radius: 27, child: Icon(icon)), title: Text(title, style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text(subtitle), trailing: const Icon(Icons.chevron_right_rounded)),
  );
}
...
class Setup extends StatefulWidget {
  const Setup({super.key, required this.store});
  final LocalStore store;
...
class _SetupState extends State<Setup> {
  ...
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('Regular Study')), body: ListView(padding: const EdgeInsets.all(20), children: [
    Text('Plan your focus time', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)), const SizedBox(height: 6), const Text('Set your total time first. You can never allocate more than this.'), const SizedBox(height: 22),
    Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Total study time', style: TextStyle(fontWeight: FontWeight.w800)), Text('$total min', style: const TextStyle(fontWeight: FontWeight.w900))]), Slider(value: total.toDouble(), min: 25, max: 480, divisions: 91, label: '$total min', onChanged: (v) => setState(() => total = v.round())), Text('${total ~/ 60}h ${total % 60}m available')]))),
    const SizedBox(height: 20), Text('Subjects', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)), const SizedBox(height: 10),
    Row(children: [Expanded(child: TextField(controller: input, onSubmitted: (_) => add(), decoration: const InputDecoration(hintText: 'e.g. Mathematics'))), const SizedBox(width: 10), FilledButton(onPressed: add, child: const Icon(Icons.add_rounded))]), const SizedBox(height: 12),
    if (subjects.isNotEmpty) Wrap(spacing: 8, runSpacing: 8, children: subjects.map((s) => InputChip(label: Text(s), onDeleted: () => setState(() => subjects.remove(s))).toList()),
    if (subjects.isEmpty) const Card(child: Padding(padding: EdgeInsets.all(16), child: Text('Add at least one subject to continue.'))),
    const SizedBox(height: 24), SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: subjects.isEmpty ? null : () => Navigator.push(context, MaterialPageRoute(builder: (_) => RegularStudyPlanner(store: widget.store, total: total, subjects: List<String>.from(subjects)))), icon: const Icon(Icons.arrow_forward_rounded), label: const Text('Continue to topic allocation')),
  ]));
}

class Allocation extends StatefulWidget {
  const Allocation({super.key, required this.store, required this.total, required this.subjects});
  final LocalStore store;
  final int total;
  final List<String> subjects;
  @override State<Allocation> createState() => _AllocationState();
}
...
class _AllocationState extends State<Allocation> {
  ...
  void initState() {
    ...
    minutes = List.generate(widget.subjects.length, (i) => base + (i < extra ? 1 : 0));
    for (final s in widget.subjects) { topics[s] = []; controllers[s] = TextEditingController(); }
  }
  @override void dispose() { for (final c in controllers.values) { c.dispose(); } super.dispose(); }
  int get allocated => minutes.fold(0, (a, b) => a + b);
  int get remaining => widget.total - allocated;
  void change(int i, int value) { final others = allocated - minutes[i]; final max = widget.total - others; setState(() => minutes[i] = value.clamp(0, max)); }
  void addTopic(String s) { final c = controllers[s]!; final t = c.text.trim(); if (t.isNotEmpty && !topics[s]!.contains(t)) setState(() { topics[s]!.add(t); c.clear(); }); }
  void save() async {
    final items = <StudyItem>[];
    for (var i = 0; i < widget.subjects.length; i++) { final subject = widget.subjects[i]; final m = minutes[i]; if (m <= 0) continue; final ts = topics[subject]!; items.add(StudyItem(title: subject, minutes: m, topic: ts.join(' • '))); }
    if (items.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Allocate at least 1 minute to a subject.'))); return; }
