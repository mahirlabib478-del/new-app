...
/// Lightweight focus flow: simple Material surfaces, bounded content and no
/// decorative animation so timer interactions stay responsive on low-end devices.
...
class _FocusScreenState extends State<FocusScreen> with WidgetsBindingObserver {
  ...
  void _startTimer() {
    ...
    timer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (!mounted || !running || transitioning || leaving) return;
  ...
  Future<void> _persistTimerState() async {
    ...
    await widget.store.saveFocusTimerState(FocusTimerState(index: activeIndex, blockIndex: activeBlockIndex, remainingSeconds: seconds, running: running, deadlineMillis: deadline));
  }

  Future<void> _persistAndArchive() async {
    if (transitioning || leaving) return;
    await widget.store.setPlanPosition(activeIndex, activeBlockIndex);
    await _persistTimerState();
    if (!transitioning && !leaving) await StudySessionStore(widget.store).archiveCurrentPlan();
  }

  Future<bool> _handleBack() async {
    if (transitioning || leaving) return false;
    leaving = true;
    timer?.cancel();
    try {
      await widget.store.setPlanPosition(activeIndex, activeBlockIndex);
  ...
  void _popAfterPersist() {
    if (!mounted || leaving) return;
    unawaited(_handleBack().then((allow) {
  ...
  void _toggleRunning() {
    if (transitioning || leaving || seconds <= 0) return;
    setState(() => running = !running);
    timer?.cancel();
    unawaited(_persistAndArchive().then((_) {
      if (mounted && !transitioning && !leaving && running) _startTimer();
    }));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      canPop: false,
      onPopInvokedWithResult: (didPop, result) { if (!didPop) _popAfterPersist(); },
      child: Scaffold(
        ...
        body: SafeArea(
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              final timerSize = compact ? 232.0 : 260.0;
              final ringSize = timerSize - 20;
              return SingleChildScrollView(
                padding: EdgeInsets.fromLTRB(compact ? 16 : 22, 16, compact ? 16 : 22, 28),
                child: Center(
                    ...
                    child: Column(
                      children: [
                        ...
                        if (item.topic.isNotEmpty) ...[
                          ...
                          Text(item.topic, textAlign: TextAlign.center, style: TextStyle(color: scheme.onSurfaceVariant, fontWeight: FontWeight.w600)),
                        ],
                          ...
                          child: Padding(
                            ...
                            child: Row(
                              children: [
                                ...
                                const Expanded(child: Text('Topics completed', style: TextStyle(fontWeight: FontWeight.w800))),
                                Text('$completedTopicCount / $totalTopicCount', style: TextStyle(fontWeight: FontWeight.w900, color: scheme.primary)),
                              ],
                            ),
                          ),
                        ),
                        ...
                        Container(
                          ...
                          child: Stack(
                            ...
                            children: [
                              SizedBox(width: ringSize, height: ringSize, child: CircularProgressIndicator(value: progress, strokeWidth: 9, strokeCap: StrokeCap.round)),
                              Column(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
                                  Text(clock, style: (compact ? Theme.of(context).textTheme.displaySmall : Theme.of(context).textTheme.displayMedium)?.copyWith(fontWeight: FontWeight.w900)),
                                  const SizedBox(height: 3),
                                  Text(running ? 'Stay with one task' : 'Timer paused', style: TextStyle(color: scheme.onSurfaceVariant, fontWeight: FontWeight.w600)),
                                ],
                              ),
                            ],
                          ),
                        ...
                        const SizedBox(height: 16),
                        Text('$currentBlockMinutes min focus block', style: const TextStyle(fontWeight: FontWeight.w800)),
                        const SizedBox(height: 18),
                        SizedBox(
                          width: double.infinity,
                          child: FilledButton.icon(
                            onPressed: transitioning || leaving ? null : _toggleRunning,
                            icon: Icon(running ? Icons.pause_rounded : Icons.play_arrow_rounded),
                            label: Text(running ? 'Pause focus' : 'Resume focus'),
                          ),
                        ),
                        ...
                        SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            onPressed: transitioning || leaving || elapsedMinutes < 1 ? null : () => unawaited(_openBreak(elapsedMinutes)),
                        ...
                        Card(
                          child: Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                Icon(Icons.save_outlined, size: 20, color: scheme.primary),
                                const SizedBox(width: 10),
                                const Expanded(child: Text('Your session is saved locally. If the app closes, your latest topic progress and position are kept in Saved sessions.')),
                              ],
                            ),
                          ),
                        ),
...
class _BreakScreenState extends State<BreakScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    unawaited(_persistBreakAndArchive());
    if (expiredOnResume) {
  ...
  void _startTimer() {
    timer?.cancel();
    timer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (!mounted || !running || advancing || leaving) return;
      if (seconds > 0) setState(() => seconds--);
      if (seconds == 0) {
        timer?.cancel();
        unawaited(_next());
  ...
  Future<void> _persistBreakState() async {
    if (advancing || leaving) return;
    final deadline = running ? DateTime.now().add(Duration(seconds: seconds)).millisecondsSinceEpoch : null;
    await widget.store.saveBreakTimerState(BreakTimerState(index: widget.index, blockIndex: widget.blockIndex, breakMinutes: breakMinutes, remainingSeconds: seconds, running: running, deadlineMillis: deadline));
  }

  Future<void> _persistBreakAndArchive() async {
    if (advancing || leaving) return;
    await _persistBreakState();
    if (!advancing && !leaving) await StudySessionStore(widget.store).archiveCurrentPlan();
  }

  Future<bool> _handleBack() async {
    if (advancing || leaving) return false;
    leaving = true;
    timer?.cancel();
    try {
      final deadline = running ? DateTime.now().add(Duration(seconds: seconds)).millisecondsSinceEpoch : null;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused || state == AppLifecycleState.inactive) {
      if (!leaving && !advancing) unawaited(_persistBreakAndArchive());
    } else if (state == AppLifecycleState.resumed && mounted && running && !advancing && !leaving) {
      final saved = widget.store.breakTimerState;
  ...
  void _setBreakMinutes(int minutes) {
    if (advancing || leaving || seconds < breakMinutes * 60 - 5) return;
    setState(() {
  ...
  void _toggleRunning() {
    if (advancing || leaving || seconds <= 0) return;
    setState(() => running = !running);
    timer?.cancel();
    unawaited(_persistBreakAndArchive().then((_) {
      if (mounted && !advancing && !leaving && running) _startTimer();
    }));
  }
          ...
          child: SingleChildScrollView(
            ...
            child: Center(
              child: ConstrainedBox(
                ...
                child: Column(
                  children: [
                    Icon(Icons.spa_rounded, size: 52, color: scheme.primary),
                    const SizedBox(height: 12),
                    Text('Break time', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)),
                    const SizedBox(height: 6),
                    Text('You completed ${widget.completed} minute${widget.completed == 1 ? '' : 's'}. Reset before you return.', textAlign: TextAlign.center, style: TextStyle(color: scheme.onSurfaceVariant)),
                    const SizedBox(height: 18),
                    Card(
                      child: Padding(
                        padding: const EdgeInsets.all(14),
                        child: Column(
                          children: [
                            ...
                            Row(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                ChoiceChip(label: const Text('5 min'), selected: breakMinutes == 5, onSelected: advancing || leaving ? null : (_) => _setBreakMinutes(5)),
                                const SizedBox(width: 8),
                                ChoiceChip(label: const Text('10 min'), selected: breakMinutes == 10, onSelected: advancing || leaving ? null : (_) => _setBreakMinutes(10)),
                              ],
                            ),
                          ],
                        ),
                    ...
                    const Card(
                      child: Column(
                        children: [
                          ...
                          ListTile(leading: Icon(Icons.directions_walk_rounded), title: Text('Walk or stretch'), subtitle: Text('Give your body a short reset.')),
                          ListTile(leading: Icon(Icons.visibility_rounded), title: Text('Rest your eyes'), subtitle: Text('Look away from the screen.')),
                          ListTile(leading: Icon(Icons.air_rounded), title: Text('Take slow breaths'), subtitle: Text('Relax your shoulders and jaw.')),
                        ],
                      ),
                    ),
                    const SizedBox(height: 16),
                    SizedBox(
                      width: double.infinity,
                      child: FilledButton(onPressed: advancing || leaving ? null : _next, child: Text(advancing || leaving ? 'Saving…' : 'Continue')),
                    ),
                    const SizedBox(height: 8),
                    SizedBox(
                      width: double.infinity,
                      child: OutlinedButton(onPressed: advancing || leaving ? null : _toggleRunning, child: Text(running ? 'Pause break' : 'Resume break')),
                    ),
                  ],
                ),
              ),
        ...
        child: Center(
          child: SingleChildScrollView(
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                children: [
                  Icon(Icons.emoji_events_rounded, size: 68, color: scheme.primary),
                  const SizedBox(height: 16),
                  Text('Study complete', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)),
                  const SizedBox(height: 7),
                  ...
                  SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: () => Navigator.popUntil(context, (route) => route.isFirst), icon: const Icon(Icons.home_rounded), label: const Text('Back to home'))),
                ],
              ),
            ),
          ),
...
class _Stat extends StatelessWidget {
  ...
  final String value;
  @override
  Widget build(BuildContext context) => Column(children: [Text(value, style: const TextStyle(fontWeight: FontWeight.w900)), const SizedBox(height: 4), Text(label)]);
}
