...
interface StudyPlanDao {
    @Query("SELECT * FROM study_plans WHERE isCompleted = 0 AND isDraft = 0 AND isArchived = 0 ORDER BY lastUpdated DESC LIMIT 1")
    ...
    @Query("UPDATE study_plans SET remainingSecondsInBlock = :remainingSec, isBreakPhase = :isBreak, currentBlockIndex = :blockIndex, isTimerRunning = :isRunning, endAtElapsedRealtime = :endAtElapsedRealtime, endAtWallClockMillis = :endAtWallClockMillis, lastUpdated = :timestamp WHERE id = :planId")
    suspend fun updateSessionTimer(planId: Long, remainingSec: Int, isBreak: Boolean, blockIndex: Int, isRunning: Boolean, endAtElapsedRealtime: Long, endAtWallClockMillis: Long, timestamp: Long = System.currentTimeMillis())
