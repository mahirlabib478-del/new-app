...
class _StudyOSState extends State<StudyOS> {
  ...
  void startPlan(StudyPlan plan) => Navigator.of(context).push(MaterialPageRoute(builder: (_) => FocusScreen(store: widget.store, plan: plan, index: 0, blockIndex: 0)));
  ...
  @override Widget build(BuildContext context) => MaterialApp(
    ...
    home: Scaffold(
      body: [
        Home(store: widget.store, onTheme: () async { setState(() => dark = !dark); await widget.store.setDarkMode(dark); }),
...
class Home extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(body: SafeArea(child: ListView(padding: const EdgeInsets.all(20), children: [
      ...
      if (snapshot.hasPlan) _NextUp(snapshot: snapshot, onStart: () => Navigator.push(context, MaterialPageRoute(builder: (_) => FocusScreen(store: store, plan: plan!, index: 0, blockIndex: 0)))),
      ...
      Card(child: ListTile(
        leading: const Icon(Icons.event_note_rounded, size: 30),
        title: Text(plan == null ? 'No plan yet' : '${plan.items.length} focus blocks planned'),
        subtitle: Text(plan == null ? 'Create a plan to get started.' : '${snapshot.remainingMinutes} minutes remaining'),
        trailing: plan == null ? null : const Icon(Icons.arrow_forward_rounded),
        onTap: plan == null ? null : () => Navigator.push(context, MaterialPageRoute(builder: (_) => FocusScreen(store: store, plan: plan, index: 0, blockIndex: 0))),
      )),
    ])));
  }
}
...
class _Hero extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(padding: const EdgeInsets.all(22), decoration: BoxDecoration(gradient: LinearGradient(colors: [c.primary, c.secondary]), borderRadius: BorderRadius.circular(28)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
