...
class _ProgressDashboardState extends State<ProgressDashboard> {
  Future<void> _changeGoal() async {
    final value = await showDialog<int>(
      context: context,
      builder: (_) => _DailyGoalDialog(initialGoal: widget.store.dailyGoalMinutes),
    );
...
class _DailyGoalDialogState extends State<_DailyGoalDialog> {
  ...
  @override Widget build(BuildContext context) => AlertDialog(
    ...
    actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')), FilledButton(onPressed: () => Navigator.pop(context, int.tryParse(controller.text.trim())), child: const Text('Save'))],
  );
}

class _DayBar extends StatelessWidget {
  const _DayBar({required this.date, required this.minutes, required this.goal});
  final DateTime date; final int minutes; final int goal;
  @override Widget build(BuildContext context) {
    final ratio = goal <= 0 ? 0.0 : (minutes / goal).clamp(0.0, 1.0).toDouble();
    final height = 16 + ratio * 92; final isToday = DateUtils.isSameDay(date, DateTime.now());
