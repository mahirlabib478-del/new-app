...
class _FocusScreenState extends State<FocusScreen> with WidgetsBindingObserver {
  ...
  void _startTimer() { timer?.cancel(); timer = Timer.periodic(const Duration(seconds: 1), (_) { if (!mounted || !running || transitioning || leaving) return; if (seconds > 0) setState(() => seconds--); if (seconds == 0) { timer?.cancel(); unawaited(_openBreak(currentBlockMinutes)); } }); }
  Future<void> _persistTimerState() async { if (transitioning || leaving) return; final deadline = running ? DateTime.now().add(Duration(seconds: seconds)).millisecondsSinceEpoch : null; await widget.store.saveFocusTimerState(FocusTimerState(index: activeIndex, blockIndex: activeBlockIndex, remainingSeconds: seconds, running: running, deadlineMillis: deadline)); }
  Future<void> _persistAndArchive() async { if (transitioning || leaving) return; await widget.store.setPlanPosition(activeIndex, activeBlockIndex); await _persistTimerState(); if (!transitioning && !leaving) await StudySessionStore(widget.store).archiveCurrentPlan(); }
  Future<bool> _handleBack() async { if (transitioning || leaving) return false; leaving = true; timer?.cancel(); try { await widget.store.setPlanPosition(activeIndex, activeBlockIndex); final deadline = running ? DateTime.now().add(Duration(seconds: seconds)).millisecondsSinceEpoch : null; await widget.store.saveFocusTimerState(FocusTimerState(index: activeIndex, blockIndex: activeBlockIndex, remainingSeconds: seconds, running: running, deadlineMillis: deadline)); await StudySessionStore(widget.store).archiveCurrentPlan(); return true; } catch (_) { leaving = false; rethrow; } }
  void _popAfterPersist() { if (!mounted || leaving) return; unawaited(_handleBack().then((allow) { if (allow && mounted) Navigator.of(context).pop(); }).catchError((_) {})); }

  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused || state == AppLifecycleState.inactive) {
      if (!leaving && !transitioning) unawaited(_persistAndArchive());
    } else if (state == AppLifecycleState.resumed && mounted && running && !transitioning && !leaving) {
      final saved = widget.store.focusTimerState;
  ...
  void _toggleRunning() { if (transitioning || leaving || seconds <= 0) return; setState(() => running = !running); timer?.cancel(); unawaited(_persistAndArchive().then((_) { if (mounted && !transitioning && !leaving && running) _startTimer(); })); }

  Future<void> _showCompletion() async {
    if (!mounted || transitioning || leaving) return;
  ...
  Future<void> _openBreak(int completed) async {
    if (transitioning || leaving || !mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(canPop: false, onPopInvokedWithResult: (didPop, result) { if (!didPop) _popAfterPersist(); }, child: Scaffold(appBar: AppBar(title: const Text('Focus mode')), body: SafeArea(child: Center(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(24, 20, 24, 30), child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 560), child: Column(children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.center, children: [FilledButton.icon(onPressed: transitioning || leaving ? null : _toggleRunning, icon: Icon(running ? Icons.pause_rounded : Icons.play_arrow_rounded), label: Text(running ? 'Pause' : 'Resume')), const SizedBox(width: 12), OutlinedButton.icon(onPressed: transitioning || leaving || elapsedMinutes < 1 ? null : () => unawaited(_openBreak(elapsedMinutes)), icon: const Icon(Icons.done_rounded), label: const Text('Finish early'))]), const SizedBox(height: 22),
...
class _BreakScreenState extends State<BreakScreen> with WidgetsBindingObserver {
  int breakMinutes = 5; int seconds = 300; bool running = true; bool advancing = false; bool leaving = false; Timer? timer;
  ...
  void _startTimer() { timer?.cancel(); timer = Timer.periodic(const Duration(seconds: 1), (_) { if (!mounted || !running || advancing || leaving) return; if (seconds > 0) setState(() => seconds--); if (seconds == 0) { timer?.cancel(); unawaited(_next()); } }); }
  Future<void> _persistBreakState() async { if (advancing || leaving) return; final deadline = running ? DateTime.now().add(Duration(seconds: seconds)).millisecondsSinceEpoch : null; await widget.store.saveBreakTimerState(BreakTimerState(index: widget.index, blockIndex: widget.blockIndex, breakMinutes: breakMinutes, remainingSeconds: seconds, running: running, deadlineMillis: deadline)); }
  Future<void> _persistBreakAndArchive() async { if (advancing || leaving) return; await _persistBreakState(); if (!advancing && !leaving) await StudySessionStore(widget.store).archiveCurrentPlan(); }
  Future<bool> _handleBack() async { if (advancing || leaving) return false; leaving = true; timer?.cancel(); try { final deadline = running ? DateTime.now().add(Duration(seconds: seconds)).millisecondsSinceEpoch : null; await widget.store.saveBreakTimerState(BreakTimerState(index: widget.index, blockIndex: widget.blockIndex, breakMinutes: breakMinutes, remainingSeconds: seconds, running: running, deadlineMillis: deadline)); await StudySessionStore(widget.store).archiveCurrentPlan(); return true; } catch (_) { leaving = false; rethrow; } }
  void _popAfterPersist() { if (!mounted || leaving) return; unawaited(_handleBack().then((allow) { if (allow && mounted) Navigator.of(context).pop(); }).catchError((_) {})); }
  @override void didChangeAppLifecycleState(AppLifecycleState state) { if (state == AppLifecycleState.paused || state == AppLifecycleState.inactive) { if (!leaving && !advancing) unawaited(_persistBreakAndArchive()); } else if (state == AppLifecycleState.resumed && mounted && running && !advancing && !leaving) { final saved = widget.store.breakTimerState; if (saved != null && saved.index == widget.index && saved.blockIndex == widget.blockIndex && saved.deadlineMillis != null) { final nextSeconds = ((saved.deadlineMillis! - DateTime.now().millisecondsSinceEpoch) / 1000).ceil(); if (nextSeconds <= 0) { seconds = 0; timer?.cancel(); unawaited(_next()); return; } setState(() => seconds = nextSeconds.clamp(0, breakMinutes * 60).toInt()); } _startTimer(); } }
  void _setBreakMinutes(int minutes) { if (advancing || leaving || seconds < breakMinutes * 60 - 5) return; setState(() { breakMinutes = minutes; seconds = minutes * 60; }); unawaited(_persistBreakAndArchive()); }
  void _toggleRunning() { if (advancing || leaving || seconds <= 0) return; setState(() => running = !running); timer?.cancel(); unawaited(_persistBreakAndArchive().then((_) { if (mounted && !advancing && !leaving && running) _startTimer(); })); }
  Future<void> _next() async {
    if (advancing || leaving) return; setState(() => advancing = true); timer?.cancel(); await widget.store.clearBreakTimerState();
